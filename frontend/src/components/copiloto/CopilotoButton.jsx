...
/**
 * CopilotoButton - Botón Flotante Animado y Movible para el Chat de Skynet
 *
 * Características:
 * - Icono vectorial animado giratorio multi-capa con fondo 100% transparente.
 * - Tamaño de 70px por defecto (más grande e imponente).
 * - Detección de Tap/Clic integrada con Framer Motion (100% confiable).
 * - Adaptación dinámica automática para Modo Claro y Modo Oscuro.
 * - Efectos de resplandor neón, aura pulsante y ondas expansivas al interactuar.
 */
export function CopilotoButton({
  ...
  return (
    <div className={cn('relative inline-flex items-center justify-center group select-none', className)}>
      ...
      {showTooltip && !isOpen && (
        <div
          className={cn(
            ...
            // Modo Oscuro: Vidrio neón cian
            'dark:bg-slate-950/90 dark:text-cyan-300 dark:border dark:border-cyan-500/40 dark:shadow-[0_0_22px_rgba(6,182,212,0.3)]',
            // Modo Claro: Vidrio azul elegante
            'bg-slate-900/90 text-sky-100 border border-sky-400/30 shadow-sky-900/25',
      ...
      {/* Botón Flotante Principal con motion.button y onTap para respuesta instantánea */}
      <motion.button
        type="button"
        onTap={handleTap}
        ...
        className={cn(
          ...
          // Fondo 100% Transparente con sutil resplandor de fondo en hover
          'bg-transparent hover:bg-cyan-500/15 dark:hover:bg-cyan-400/20',
          // Sombras de resplandor Neón según el tema
          'dark:drop-shadow-[0_0_20px_rgba(34,211,238,0.6)] drop-shadow-[0_0_15px_rgba(2,132,199,0.45)]',
          'hover:drop-shadow-[0_0_30px_rgba(34,211,238,0.9)] hover:scale-105 active:scale-95',
          isOpen && 'rotate-90'
        )}
        style={{ width: size, height: size }}
      >
        {/* Renderizado de Ícono: Alterna suavemente entre el SVG sagrado animado y la X al abrir */}
        {isOpen ? (
        ...
        {/* Punto Indicador de Estado Activo (Dot Badge) */}
        {!isOpen && (
