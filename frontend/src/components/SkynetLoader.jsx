 ...
 * Presenta una interfaz HUD táctica con la paleta cyber del login (#4e7cbe, #0d1b33, #71bd8e).
 */
export default function SkynetLoader({
  ...
  if (pantallaCompleta) {
    return (
      <div
        className={`fixed inset-0 z-50 flex items-center justify-center bg-[#0d1b33]/90 backdrop-blur-md p-4 select-none transform-gpu ${className}`}
      ...
      >
        <div className="relative flex flex-col items-center justify-center w-full max-w-sm p-6 sm:p-8 rounded-md border border-[#4e7cbe]/20 bg-[#142744]/95 shadow-2xl shadow-black/80 ring-1 ring-[#4e7cbe]/15">
          ...
          <div className={`relative ${tamanoOrb} flex items-center justify-center mb-4 transform-gpu`}>
            ...
            <div
              className="absolute inset-0 rounded-full border-2 border-transparent border-t-[#4e7cbe] border-r-[#71bd8e] animate-spin transform-gpu"
              style={{ animationDuration: '1.4s' }}
            />
            <div
              className="absolute inset-1.5 rounded-full border border-dashed border-[#4e7cbe]/60 animate-spin transform-gpu"
          ...
          <h3
            ...
            style={{ textShadow: '0 0 12px rgba(79, 124, 190, 0.4)' }}
          >
            {mensaje}
          </h3>

          {subtexto && (
            <p className="text-[11px] text-[#4e7cbe]/80 font-mono tracking-wider mt-1.5 text-center opacity-90 animate-pulse">
              {subtexto}
            </p>
          )}

          {/* Barra de progreso de escaneo */}
          <div className="w-36 h-1 bg-[#4e7cbe]/15 rounded-full overflow-hidden mt-4 relative">
            <div
              className="absolute inset-y-0 w-1/2 bg-gradient-to-r from-transparent via-[#4e7cbe] to-transparent rounded-full animate-[skynetGpuBar_1.4s_cubic-bezier(0.4,0,0.2,1)_infinite] transform-gpu"
              style={{ boxShadow: '0 0 8px #4e7cbe' }}
            />
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className={`relative ${tamanoOrb} flex items-center justify-center mb-3 transform-gpu`}>
        <div className="absolute -inset-1 rounded-full bg-[#4e7cbe]/20 blur-sm animate-pulse transform-gpu" />
        <div
          className="absolute inset-0 rounded-full border-2 border-transparent border-t-[#4e7cbe] border-r-[#71bd8e] animate-spin transform-gpu"
          style={{ animationDuration: '1.2s' }}
        />
        <div
          className={`${tamanoCore} bg-[#101f38] border border-[#4e7cbe]/50 flex items-center justify-center shadow-[0_0_12px_rgba(79, 124, 190,0.3)]`}
        >
          <span className="font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white via-[#d7e4f4] to-[#4e7cbe]">
            S
          </span>
        </div>
      </div>
      ...
      {mensaje && (
        <p
          className="text-xs font-semibold tracking-[0.2em] text-[#4e7cbe] uppercase text-center animate-pulse font-mono"
          style={{ textShadow: '0 0 8px rgba(79, 124, 190, 0.4)' }}
        >
          {mensaje}
        </p>
      )}
